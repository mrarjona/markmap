...
## Exporting a high-resolution PNG

After generating the HTML, render it to a PNG with headless Chromium:
...
| Argument | Default | Description |
|---|---|---|
| `input.html` | `demo/bidirectional-demo.html` | Path to the HTML file generated by `generate.mjs` |
| `output.png` | `demo/mindmap.png` | Path where the PNG file is written |

### Default (uses `demo/bidirectional-demo.html`)

```bash
node demo/export-png.mjs
```
...
```bash
node demo/export-png.mjs path/to/my-map.html path/to/output.png
```
...
| Property | Value |
|---|---|
| DPI scale | 3× device pixel ratio (≈ 288 DPI on a 96 DPI screen) |
| Background | White |
| Dimensions | Auto-fitted to the bounding box of the rendered graph (no excessive margins, no cropping) |
| Chromium | Uses the system installation; override with `CHROME_PATH` env var |

### Full pipeline example

```bash
# 1. Build the packages (first time only)
pnpm install && pnpm build:types && pnpm build:js

# 2. Generate the self-contained HTML
node demo/generate.mjs
